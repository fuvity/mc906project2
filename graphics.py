...
# Pegando parametros
heightmap_size = 2000 #int(input('Tamanho do mapa:'))
max_iterations = 1000 #int(input('Numero maximo de iterações:))
pop_size = 30#int(input('Tamanho da População: '))
fittest_selection = 20#int(input('Quantos se reproduzirão por geração: '))
fittest_selection = int(pop_size*fittest_selection/100)
mutation_chance = 3#int(input('Chance de Mutação(%): '))
...
choice_stop = 1 #1 = max iterations, 2 = std dev. tem de ser 1/30 do inicial
...
if choice_stop == 1:
    ...
    deviationtarget = np.std(childrenz)/30
...
while eval(condition):
    ...
    if selection_method == 1:
        ...
        fittest = children[(pop_size-fittest_selection):]
    else:
        keys = []
    ...
    for i in range(crossover_por_geracao): # quem faz crossover mistura os pais
        children.append(getChild(fittest[np.random.randint(len(fittest))], fittest[np.random.randint(len(fittest))]))
